    ...
    { id: 'dashboard', icon: '📊',
      ...
      steps: [
        ...
        { target: '#pipeline-overview', position: 'bottom',
          title: { ko: '파이프라인 개요', en: 'Pipeline Overview', ja: 'パイプライン概要', 'zh-CN': '管道概览', 'zh-TW': '管道概覽', es: 'Resumen del pipeline' },
          content: { ko: '현재 <strong>실행 중인 파이프라인</strong>의 정보(모델명, 해상도, 상태)를 표시합니다.', en: 'Shows information about the <strong>currently running pipeline</strong> (model, resolution, status).', ja: '現在<strong>実行中のパイプライン</strong>の情報(モデル名、解像度、状態)を表示します。', 'zh-CN': '显示当前<strong>运行中管道</strong>的信息（模型名称、分辨率、状态）。', 'zh-TW': '顯示當前<strong>執行中管道</strong>的資訊（模型名稱、解析度、狀態）。', es: 'Muestra información del <strong>pipeline en ejecución</strong> (modelo, resolución, estado).' } },
        { target: '#quick-launch-grid', position: 'bottom',
          title: { ko: '빠른 실행', en: 'Quick Launch', ja: 'クイックランチ', 'zh-CN': '快速启动', 'zh-TW': '快速啟動', es: 'Inicio rápido' },
    ...
    { id: 'demo', icon: '🎬',
      ...
      steps: [
        ...
        { target: '#dx-input-modal', position: 'bottom',
          ...
          content: { ko: '카메라·RTSP URL·파일 경로·sudo 비밀번호 등을 묻는 <strong>접근 가능한 입력 dialog</strong>입니다. 브라우저 <code>prompt()</code> 대신 사용하며, 데모·파이프라인·Setup에서 나타납니다.', en: 'An <strong>accessible input dialog</strong> for camera names, RTSP URLs, file paths, sudo passwords, and more — used instead of browser <code>prompt()</code> in demos, the pipeline builder, and Setup.', ja: 'カメラ名・RTSP URL・ファイルパス・sudoパスワードなどを尋ねる<strong>アクセシブルな入力dialog</strong>です。ブラウザの<code>prompt()</code>の代わりにデモ・パイプライン・Setupで表示されます。', 'zh-CN': '用于询问摄像头名称、RTSP URL、文件路径、sudo 密码等的<strong>无障碍输入对话框</strong>，在演示、管道构建器和 Setup 中替代浏览器 <code>prompt()</code>。', 'zh-TW': '用於詢問攝影機名稱、RTSP URL、檔案路徑、sudo 密碼等的<strong>無障礙輸入對話框</strong>，在示範、管道建構器和 Setup 中替代瀏覽器 <code>prompt()</code>。', es: 'Un <strong>diálogo de entrada accesible</strong> para nombres de cámara, URL RTSP, rutas de archivo, contraseñas sudo y más; sustituye a <code>prompt()</code> del navegador en demos, el constructor de pipelines y Setup.' },
          beforeStep: function () {
        ...
        { target: '#demo-grid [id^="start-demo-"]', position: 'right',
          ...
          content: { ko: '▶ 버튼을 클릭하면 파이프라인이 시작됩니다. 실행 중인 카드에는 <strong>초록 테두리 + pulse 애니메이션</strong>이 표시됩니다.', en: 'Click ▶ to start the pipeline. Running cards show a <strong>green border + pulse animation</strong>.', ja: '▶をクリックするとパイプラインが開始します。実行中のカードには<strong>緑枠 + パルスアニメーション</strong>が表示されます。', 'zh-CN': '点击▶启动管道。运行中的卡片显示<strong>绿色边框 + 脉冲动画</strong>。', 'zh-TW': '點擊▶啟動管道。執行中的卡片顯示<strong>綠色邊框 + 脈衝動畫</strong>。', es: 'Haga clic en ▶ para iniciar el pipeline. Las tarjetas en ejecución muestran un <strong>borde verde y animación de pulso</strong>.' },
          beforeStep: function () {
        ...
        { target: '#btn-demo-stop', position: 'left',
          ...
          content: { ko: '⏹ 버튼을 클릭하면 실행 중인 <strong>데모를 중지</strong>합니다. 파이프라인이 종료되고 비디오 섹션이 닫힙니다.', en: 'Click ⏹ to <strong>stop the running demo</strong>. The pipeline stops and the video section closes.', ja: '⏹をクリックすると実行中の<strong>デモを停止</strong>します。パイプラインが終了しビデオセクションが閉じます。', 'zh-CN': '点击⏹<strong>停止运行中的演示</strong>。管道停止，视频区域关闭。', 'zh-TW': '點擊⏹<strong>停止執行中的示範</strong>。管道停止，影片區域關閉。', es: 'Haga clic en ⏹ para <strong>detener la demo en ejecución</strong>. El pipeline se detiene y se cierra la sección de vídeo.' },
          beforeStep: function () { _mockDemoVideoPreview(); } },
        { target: '#demo-pipeline-info', position: 'top',
          title: { ko: '파이프라인 정보', en: 'Pipeline Info', ja: 'パイプライン情報', 'zh-CN': '管道信息', 'zh-TW': '管道資訊', es: 'Información del pipeline' },
          content: { ko: '실행 중인 파이프라인의 <strong>모델명, 해상도, 입력 소스</strong> 등의 정보가 하단 바에 표시됩니다.', en: 'Shows <strong>model name, resolution, input source</strong> of the running pipeline in the bottom bar.', ja: '実行中のパイプラインの<strong>モデル名、解像度、入力ソース</strong>などの情報がボトムバーに表示されます。', 'zh-CN': '底部栏显示运行中管道的<strong>模型名称、分辨率、输入源</strong>等信息。', 'zh-TW': '底部欄顯示執行中管道的<strong>模型名稱、解析度、輸入來源</strong>等資訊。', es: 'Muestra en la barra inferior <strong>nombre del modelo, resolución y fuente de entrada</strong> del pipeline en ejecución.' },
          beforeStep: function () {
    ...
    { id: 'pipeline', icon: '🔧',
      ...
      steps: [
        { target: '.pipeline-actions', position: 'bottom',
          title: { ko: '파이프라인 툴바', en: 'Pipeline Toolbar', ja: 'パイプラインツールバー', 'zh-CN': '管道工具栏', 'zh-TW': '管道工具列', es: 'Barra de herramientas del pipeline' },
        ...
        { target: '#palette-search', position: 'right',
          title: { ko: '팔레트 검색', en: 'Palette Search', ja: 'パレット検索', 'zh-CN': '调色板搜索', 'zh-TW': '調色板搜尋', es: 'Búsqueda en la paleta' },
    ...
    { id: 'global', icon: '🌐',
      ...
      steps: [
        ...
        { target: '.topbar', position: 'bottom',
          ...
          content: { ko: '현재 <strong>페이지 제목</strong>과 <strong>파이프라인 실행 상태 배지</strong>가 표시됩니다.', en: 'Shows the current <strong>page title</strong> and <strong>pipeline status badge</strong>.', ja: '現在の<strong>ページタイトル</strong>と<strong>パイプライン実行状態バッジ</strong>が表示されます。', 'zh-CN': '显示当前<strong>页面标题</strong>和<strong>管道运行状态徽章</strong>。', 'zh-TW': '顯示當前<strong>頁面標題</strong>和<strong>管道執行狀態徽章</strong>。', es: 'Muestra el <strong>título de la página</strong> actual y la <strong>insignia de estado del pipeline</strong>.' } },
        { target: '#pipeline-status', position: 'bottom',
          title: { ko: '파이프라인 상태', en: 'Pipeline Status', ja: 'パイプライン状態', 'zh-CN': '管道状态', 'zh-TW': '管道狀態', es: 'Estado del pipeline' },
          content: { ko: '🟡 <strong>대기 중</strong> / 🟢 <strong>실행 중</strong> 상태를 표시합니다. 파이프라인 실행/중지 시 자동 업데이트됩니다.', en: '🟡 <strong>Idle</strong> / 🟢 <strong>Running</strong> status. Auto-updates when pipeline starts/stops.', ja: '🟡 <strong>アイドル</strong> / 🟢 <strong>実行中</strong> 状態を表示します。パイプライン開始/停止時に自動更新されます。', 'zh-CN': '🟡 <strong>空闲</strong> / 🟢 <strong>运行中</strong> 状态。管道启动/停止时自动更新。', 'zh-TW': '🟡 <strong>閒置</strong> / 🟢 <strong>執行中</strong> 狀態。管道啟動/停止時自動更新。', es: 'Estado 🟡 <strong>Inactivo</strong> / 🟢 <strong>En ejecución</strong>. Se actualiza automáticamente al iniciar/detener el pipeline.' } },
    ...
    { id: 'chat', icon: '💬',
      ...
      steps: [
        { target: '.dx-chat-fab', position: 'left',
          ...
          content: { ko: '💬 버튼을 클릭하면 <strong>AI 챗봇 창</strong>이 열립니다. 파이프라인 구성, 요소, 데모 등에 대해 질문할 수 있습니다.', en: 'Click 💬 to open the <strong>AI chatbot window</strong>. Ask about pipeline configuration, elements, demos, and more.', ja: '💬ボタンをクリックすると<strong>AIチャットボットウィンドウ</strong>が開きます。パイプライン構成、エレメント、デモなどについて質問できます。', 'zh-CN': '点击💬打开<strong>AI聊天机器人窗口</strong>。可询问管道配置、元素、演示等问题。', 'zh-TW': '點擊💬開啟<strong>AI聊天機器人視窗</strong>。可詢問管道配置、元素、示範等問題。', es: 'Haga clic en 💬 para abrir la <strong>ventana del chatbot de IA</strong>. Pregunte sobre configuración de pipelines, elementos, demos y más.' },
        ...
        { target: '.dx-chat-suggestions', position: 'left',
          ...
          content: { ko: '<strong>파이프라인 관련 추천 질문</strong>을 클릭하면 빠르게 대화를 시작할 수 있습니다.', en: 'Click <strong>pipeline-related suggestions</strong> to quickly start a conversation.', ja: '<strong>パイプライン関連のおすすめ質問</strong>をクリックすると素早く会話を始められます。', 'zh-CN': '点击<strong>管道相关推荐问题</strong>可快速开始对话。', 'zh-TW': '點擊<strong>管道相關推薦問題</strong>可快速開始對話。', es: 'Haga clic en las <strong>sugerencias relacionadas con pipelines</strong> para iniciar una conversación rápidamente.' },
          beforeStep: function () {
            liftChat('.dx-chat-window');
            var fab = document.querySelector('.dx-chat-fab');
            if (fab && !document.querySelector('.dx-chat-window.open')) fab.click();
