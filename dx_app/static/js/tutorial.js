    ...
    { id:'run-single', icon:'▶️',
      ...
      steps:[
        ...
        { target:'#r-mode', position:'right',
          ...
          content:{ ko: '<strong>Sync</strong>: 한 프레임 추론이 끝난 후 다음 프레임 처리. <strong>Async</strong>: 추론과 전처리를 파이프라인으로 병렬 처리하여 더 높은 FPS를 달성합니다.', en: '<strong>Sync</strong>: Process next frame after current completes. <strong>Async</strong>: Pipeline inference and preprocessing for higher FPS.', ja: '<strong>Sync</strong>：現在のフレーム処理完了後に次のフレームを処理。<strong>Async</strong>：推論と前処理をパイプラインで並列処理し、より高いFPSを実現します。', 'zh-CN': '<strong>Sync</strong>：当前帧完成后处理下一帧。<strong>Async</strong>：将推理与预处理流水线并行以获得更高FPS。', 'zh-TW': '<strong>Sync</strong>：當前幀完成後處理下一幀。<strong>Async</strong>：將推論與預處理以管道並行以獲得更高FPS。', es: '<strong>Sync</strong>: procesa el siguiente fotograma cuando termina el actual. <strong>Async</strong>: canaliza inferencia y preprocesamiento para mayor FPS.' } },
    ...
    { id:'modelzoo', icon:'📥',
      ...
      steps:[
        { target:'#mz-source', position:'right',
          ...
          content:{ ko: '<strong>🔒 Internal</strong>: 사내 폐쇄망 모델 저장소. <strong>🌐 Public</strong>: 공개 모델 저장소. 네트워크 환경에 따라 소스를 선택하세요.', en: '<strong>🔒 Internal</strong>: Private model repository. <strong>🌐 Public</strong>: Public model repo. Select based on network environment.', ja: '<strong>🔒 Internal</strong>：プライベートモデルリポジトリ。<strong>🌐 Public</strong>：公開モデルリポジトリ。ネットワーク環境に応じて選択してください。', 'zh-CN': '<strong>🔒 Internal</strong>：私有模型仓库。<strong>🌐 Public</strong>：公开模型仓库。根据网络环境选择。', 'zh-TW': '<strong>🔒 Internal</strong>：私有模型倉庫。<strong>🌐 Public</strong>：公開模型倉庫。依網路環境選擇。', es: '<strong>🔒 Internal</strong>: repositorio privado de modelos. <strong>🌐 Public</strong>: repositorio público. Seleccione según el entorno de red.' } },
    ...
    { id:'compiler', icon:'🛠️',
      ...
      steps:[
        { target:'#ref-filter-bar', position:'bottom',
          ...
          content:{ ko: 'Reference 필터에서 <strong>Compiler</strong> 칩을 선택하면 DX-COM 설치·ONNX 그래프·컴파일 워크플로우 문서를 볼 수 있습니다. 실제 GUI는 Launcher 상단 <strong>Compiler</strong> 탭에서 실행합니다.', en: 'Select the <strong>Compiler</strong> chip in Reference filters for DX-COM setup, ONNX graph, and compile workflow docs. Run the full GUI from the Launcher <strong>Compiler</strong> tab.', ja: 'Referenceフィルタで<strong>Compiler</strong>チップを選ぶと、DX-COMセットアップ・ONNXグラフ・コンパイルワークフローのドキュメントが表示されます。実際のGUIはLauncher上部の<strong>Compiler</strong>タブから起動します。', 'zh-CN': '在 Reference 筛选中选择 <strong>Compiler</strong> 标签可查看 DX-COM 安装、ONNX 图形和编译流程文档。完整 GUI 请从 Launcher 顶部 <strong>Compiler</strong> 标签启动。', 'zh-TW': '在 Reference 篩選取選擇 <strong>Compiler</strong> 標籤可查看 DX-COM 安裝、ONNX 圖形和編譯流程文件。完整 GUI 請從 Launcher 頂部 <strong>Compiler</strong> 標籤啟動。', es: 'Seleccione el chip <strong>Compiler</strong> en los filtros Reference para documentación de DX-COM, gráfico ONNX y flujo de compilación. Ejecute la GUI completa desde la pestaña <strong>Compiler</strong> del Launcher.' } },
