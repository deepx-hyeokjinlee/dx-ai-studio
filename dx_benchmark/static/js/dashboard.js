...
function renderDetailTables() {
  ...
  target.innerHTML=tasks.map(function(task){
    ...
    var body=models.map(function(model){var ortRows=[true,false].filter(function(useOrt){return state.detailOrt==='all'||state.detailOrt===(useOrt?'on':'off');});var fpsOn=null,fpsOff=null,capOn=null,capOff=null;ortRows.forEach(function(useOrt){var k=model+'|'+(useOrt?'on':'off');var e2e=e2eMap[k]||{};if(useOrt){fpsOn=e2e.avg_e2e_fps!=null?Number(e2e.avg_e2e_fps):null;capOn=capMap[k]!=null?Number(capMap[k]):null;}else{fpsOff=e2e.avg_e2e_fps!=null?Number(e2e.avg_e2e_fps):null;capOff=capMap[k]!=null?Number(capMap[k]):null;}});var bestFpsOrt=null,bestCapOrt=null;if(fpsOn!=null&&fpsOff!=null){bestFpsOrt=fpsOn>=fpsOff?'on':'off';}else if(fpsOn!=null){bestFpsOrt='on';}else if(fpsOff!=null){bestFpsOrt='off';}if(capOn!=null&&capOff!=null){bestCapOrt=capOn>=capOff?'on':'off';}else if(capOn!=null){bestCapOrt='on';}else if(capOff!=null){bestCapOrt='off';}var sz=modelSizeChar(model);var szLabel=sz?sz.toUpperCase():'-';return ortRows.map(function(useOrt){var k=model+'|'+(useOrt?'on':'off');var e2e=e2eMap[k]||{};var ortKey=useOrt?'on':'off';var fpsVal=fmt(e2e.avg_e2e_fps,1);var capVal=capMap[k]!=null?capMap[k]:null;var fpsTd=bestFpsOrt===ortKey?'<td class="cell-best">'+fpsVal+'</td>':'<td>'+fpsVal+'</td>';var capTd=bestCapOrt===ortKey?'<td class="cell-best">'+(capVal!=null?capVal:'-')+'</td>':'<td>'+(capVal!=null?capVal:'-')+'</td>';var th=thermMap[k]||{};var tempS=_fmtTemp(th.tMin,th.tMax);var clkS=_fmtClock(th.cMin,th.cMax);var tempTd='<td>'+tempS+(th.throttled?' <span class="tag tag--warn" title="'+_t('NPU thermal throttling detected')+'">'+_t('Throttled')+'</span>':'')+'</td>';return '<tr><td>'+escHtml(model)+'</td><td>'+szLabel+'</td><td>'+(useOrt?'ON':'OFF')+'</td><td>'+fmt(latMap[k],2)+'</td><td>'+fmt(tpMap[k],1)+'</td>'+fpsTd+capTd+tempTd+'<td>'+clkS+'</td></tr>';}).join('');}).join('');
    return '<section class="task-section"><h3>'+(TASK_MAP[task]?TASK_MAP[task].label:task)+'</h3><table class="summary-table detail-table"><colgroup><col style="width:22%"><col style="width:6%"><col style="width:6%"><col style="width:12%"><col style="width:13%"><col style="width:11%"><col style="width:12%"><col style="width:10%"><col style="width:8%"></colgroup><thead><tr><th>' + _t('Model') + '</th><th>' + _t('Size') + '</th><th>ORT</th><th>' + _t('Latency (ms)') + '</th><th>' + _t('Throughput') + ' (FPS)</th><th>' + _t('E2E FPS') + '</th><th>' + _t('Max Channels') + '</th><th>' + _t('NPU Temp (°C)') + '</th><th>' + _t('NPU Clock (MHz)') + '</th></tr></thead><tbody>'+body+'</tbody></table></section>';
  }).join('');
  dispatchBenchmarkHelpSync();
}
