...
(function () {
  ...
  var graphViewerSteps = [
    ...
    { target: '#zoom-in', position: 'top',
      ...
      content: { en: 'Click to zoom into the graph.', ko: '클릭하여 그래프를 확대합니다.', ja: 'クリックしてグラフを拡大します。', 'zh-CN': '点击放大图形。', 'zh-TW': '點擊放大圖形。', es: 'Haga clic para acercar el gráfico.' },
    ...
    { target: '#zoom-out', position: 'top',
      ...
      content: { en: 'Click to zoom out of the graph.', ko: '클릭하여 그래프를 축소합니다.', ja: 'クリックしてグラフを縮小します。', 'zh-CN': '点击缩小图形。', 'zh-TW': '點擊縮小圖形。', es: 'Haga clic para alejar el gráfico.' },
    ...
    { target: '#zoom-fit', position: 'top',
      ...
      content: { en: 'Fit the entire graph into the visible area.', ko: '전체 그래프를 화면에 맞춰 표시합니다.', ja: 'グラフ全体を表示領域に収めます。', 'zh-CN': '将整个图形适应到可见区域。', 'zh-TW': '將整個圖形適應到可見區域。', es: 'Ajuste todo el gráfico al área visible.' },
    ...
    { target: '#explorer-toggle', position: 'right', title: { en: 'Explorer', ko: '익스플로러', ja: 'エクスプローラー', 'zh-CN': '资源管理器', 'zh-TW': '資源管理員', es: 'Explorador' }, content: { en: 'Browse nodes by category (Compute/Memory/Activation etc).', ko: '익스플로러를 열어 노드를 카테고리별로 탐색합니다.', ja: 'カテゴリ別にノードを参照します（Compute/Memory/Activationなど）。', 'zh-CN': '按类别浏览节点（Compute/Memory/Activation等）。', 'zh-TW': '按類別瀏覽節點（Compute/Memory/Activation等）。', es: 'Explore nodos por categoría (Compute/Memory/Activation, etc.).' } },
  ...
  var configWizardSteps = [
    ...
    { target: '#prep-select', position: 'bottom',
      ...
      content: { en: 'Add preprocessing transforms (resize, normalize, etc.) to the pipeline.', ko: 'resize, normalize 등 14종 전처리를 순서대로 파이프라인에 추가합니다.', ja: '前処理変換（resize、normalizeなど）をパイプラインに追加します。', 'zh-CN': '将预处理变换（resize、normalize等）添加到管线。', 'zh-TW': '將預處理轉換（resize、normalize等）新增到管線。', es: 'Añada transformaciones de preprocesamiento (resize, normalize, etc.) a la canalización.' },
      beforeStep: function () { openWizardStep(3); } },
    ...
    { target: '#wiz-prev', position: 'top',
      ...
      content: { en: 'Go back to the previous wizard step.', ko: '이전 마법사 단계로 돌아갑니다.', ja: '前のウィザードステップに戻ります。', 'zh-CN': '返回上一步向导步骤。', 'zh-TW': '返回上一步精靈步驟。', es: 'Vuelva al paso anterior del asistente.' },
      beforeStep: function () { openWizardStep(2); } },
    { target: '.wizard-close', position: 'bottom',
      ...
      content: { en: 'Close the config wizard without applying.', ko: '설정을 적용하지 않고 마법사를 닫습니다.', ja: '設定を適用せずにウィザードを閉じます。', 'zh-CN': '不应用设置关闭向导。', 'zh-TW': '不套用設定關閉精靈。', es: 'Cierre el asistente de configuración sin aplicar cambios.' },
      beforeStep: function () {
        var wiz = document.getElementById('config-wizard');
        if (wiz) wiz.style.display = 'flex';
      } },
  ...
  var agenticAutoCompileSteps = [
    ...
    { target: '#auto-compile-noninteractive', position: 'left',
      ...
      content: { ko: '에이전트가 추가 질문 없이 모든 단계를 자동으로 완료합니다.', en: 'The agent completes all steps automatically without any additional questions.', ja: 'エージェントが追加の質問なしにすべてのステップを自動で完了します。', 'zh-CN': '智能体无需额外提问，自动完成所有步骤。', 'zh-TW': '智能體無需額外提問，自動完成所有步驟。', es: 'El agente completa todos los pasos automáticamente sin preguntas adicionales.' } },
  ...
  DXTutorial.create({
    ...
    sections: [
      ...
      { id: 'graph-viewer', icon: '📊',
        title: { en: '📊 Graph Viewer', ko: '📊 그래프 뷰어', ja: '📊 グラフビューア', 'zh-CN': '📊 图形查看器', 'zh-TW': '📊 圖形檢視器', es: '📊 Visor de gráficos' },
        description: { en: 'Navigate and inspect compilation graphs', ko: '컴파일 그래프를 탐색하고 상세 정보를 확인합니다', ja: 'コンパイルグラフを探索して確認します', 'zh-CN': '浏览和检查编译图形', 'zh-TW': '瀏覽和檢查編譯圖形', es: 'Navegue e inspeccione los gráficos de compilación' },
      ...
      { id: 'advanced', icon: '🔬',
        title: { en: '🔬 Advanced', ko: '🔬 고급 기능', ja: '🔬 上級', 'zh-CN': '🔬 高级功能', 'zh-TW': '🔬 進階功能', es: '🔬 Avanzado' },
