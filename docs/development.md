...
- **Python 3.8+** — that is the only prerequisite (tested on Debian 12/13, Ubuntu 20.04–26.04). The runtime has **no third-party
  dependencies** (pure standard library), so no `pip install` is needed to run the studio.
